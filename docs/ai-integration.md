...
Flutter App -> /api/generate-daily-lesson -> OpenAI Responses API -> Flutter App
```
...
每日练习生成也使用同一个后端地址。没有配置后端时，App 会使用本地推荐，并把当天 lesson 缓存到本机。
...
4. 每日练习生成：已具备接口骨架、App 调用层和本地缓存。
5. 口语评分：上传录音，后端先转写，再评分。
